...
## 이식 진행 현황 (2026-07-02, Claude 작업)

이 저장소(`JP_vocab`, 개인용 원본 앱)에 아래 순서로 이식을 완료했습니다. `index.html` 커밋
참고: `b1e1f26`(브랜치 `claude/jp-vocab-pcm-handoff-md-boswb1`).

| 섹션 | 상태 | 비고 |
|---|---|---|
| 1. 학습 대기목록 | ✅ 완료 | |
| 2. 연관단어 이동 액션 | ✅ 완료 | |
| 3. 예문 속 단어 | ✅ 완료 | Node 스크립트로 샘플 검증(동사 활용 매칭 오탐 1건 확인 — 알려진 한계, 아래 참고) |
| **4-1. TTS 보이스 고정(`api/proxy.js`)** | ❌ **미완료 — 별도 저장소 필요** | 아래 상세 참고 |
| 4-2. 터치 발음 지연 단축 | ✅ 완료 | 보이스 랜덤 prefetch 허용(4-1과 세트)은 4-1이 안 끝나 보류, in-flight 중복방지 + 현재/다음 카드 워밍만 반영 |
| 5. 흘려듣기 목록 → 상세 | ✅ 완료 | |
| 6. 흘려듣기 범위기반 랜덤 | ✅ 완료 | |
| 7. bg모드 한정 버그 | ✅ 해당 없음 | 이 앱은 처음부터 모달을 삼항연산자 밖에 둔 구조라 버그 자체가 없었음 |
| 8. WordDetailModal 최대크기 | ✅ 완료 | Playwright로 탭바 실측(58px) 후 20px 간격 직접 검증 |
| 9. 팝업 Portal 이동 | ✅ 완료 | 이 앱엔 `FeedbackBox`가 없어 `WordDetailModal`/`DeckManager` 2곳만 적용 |
| 10. 온보딩 앱셸 구조변경 | ✅ 해당 없음 | 이 앱에는 온보딩/레벨선택 화면이 없음 |
| 11. WORD_SPEECH_OVERRIDE | ✅ 완료 | 1,032건 반영, 기존 5건과 충돌 0건 |
| 12. 모달 딤 그라데이션 + 위치 | ✅ 완료 | `--dim-top` 도입 검증 완료. 스와이프-닫기 애니메이션은 이 앱에 그 제스처 자체가 없어 미적용 |

### ⚠️ 남은 일: 4-1. TTS 보이스 고정 (`api/proxy.js`)
이 부분은 `index.html`이 아니라 **별도 저장소** `jp-vocab-proxy`(`TTS_URL =
"https://jp-vocab-proxy.vercel.app/api/tts"`, `index.html`에서 참조)의 서버 코드를 고쳐야 하는
작업인데, 이번 작업 세션은 GitHub 접근 범위가 `JP_vocab`/`JP_vocab_pcm` 두 저장소로 한정되어 있어서
`jp-vocab-proxy`에는 접근하지 못해 반영하지 못했습니다.

**다음에 `jp-vocab-proxy` 저장소 접근 권한이 있는 세션에서 이어서 할 일**:
- 그 저장소의 TTS 처리 함수(이 문서의 4-1 코드 스니펫 참고, pcm의 `api/proxy.js`와 동일한 위치일 가능성 높음)에서
  Chirp3-HD 화자 선택 로직을 8개 화자 중 랜덤/지정이 아니라, **여 Aoede · 남 Charon 2개로 고정**하도록 수정
  (위 4-1의 `pickVoice` 함수 스니펫 그대로 적용 가능한지 그 저장소 구조 확인 후 적용).
- 4-1이 끝나면 `index.html`의 `prefetchAudio`에서 `if (s.gender === "random") return;`으로 되어 있는
  스킵 로직을 재검토(보이스가 2개로 고정되면 랜덤도 캐시 적중이 가능해지므로, 4-2에서 보류했던
  "랜덤도 prefetch 허용" 변경을 이때 같이 반영).
- `jp-vocab-proxy`를 접근 범위에 포함해 새 세션/환경을 만들어야 함 (현재 세션에서는 채팅으로 권한을
  부여해도 즉시 적용되지 않음 — 환경 생성/재구성 시점에 레포 목록에 추가해야 함).
